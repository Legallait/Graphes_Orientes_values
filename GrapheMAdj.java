...
public class GrapheMAdj implements IGraphe {
	...
	public String toString(){
		...
		for (int i = 0; i < matrice.length; i++) {
			if(getSucc(sommets.get(i)).size() != 0){
				for (int j = 0; j < matrice.length; j++) {
					if (matrice[i][j] != -1){
						...
						valeur = String.valueOf(matrice[i][j]);
						arcs.add(source + "-" + destination + "(" + valeur + ")");
					}
				}
			}else{
				arcs.add(sommets.get(i) + ":");
			}
		}
		arcs.sort(String::compareTo);
		String arcsStringFinal = arcs.toString();
		arcsStringFinal = arcsStringFinal.substring(1, arcsStringFinal.length() - 1);
		return arcsStringFinal;
	}
}
